...
local brightness = wibox.widget{
    ...
    bar_margins = {bottom = dpi(12), top = dpi(12)},
    bar_active_color = beautiful.fg_color,
    handle_width = dpi(0.2),
...
local function update_value_of_brightness()
...
local function create_wee_b(widget)
    widget:connect_signal("button::press", function(_, _, _, button_id)
        if button_id == 4 then  -- Scroll up
            awful.spawn("brightnessctl set 5%+", false)
            update_value_of_brightness()
        elseif button_id == 5 then  -- Scroll down
            awful.spawn("brightnessctl set 5%-", false)
            update_value_of_brightness()
        end
    end)
...
-- finalize
local sliders_widget = wibox.widget{
    {
        {
            bright_init,
            volume_init,
