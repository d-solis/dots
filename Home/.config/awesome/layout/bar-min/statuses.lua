-- minimal status indicators
----------------------------
-- Copyleft © 2022 Saimoomedits


-- requirements
---------------
local awful = require("awful")
local helpers = require("helpers")
local gears = require("gears")
local wibox = require("wibox")
local beautiful = require("beautiful")
local dpi = beautiful.xresources.apply_dpi


-- widgets
----------

-- wifi icon
local wifi = wibox.widget{
    font = beautiful.icon_var .. "12",
    markup = helpers.colorize_text(" 󰖩 ", beautiful.fg_color),
    widget = wibox.widget.textbox,
    valign = "center",
    align = "center"
}

-- bluetooth icon
local blue = wibox.widget{
    font = beautiful.icon_var .. "12",
    widget = wibox.widget.textbox,
    valign = "center",
    align = "center"
}

-- volume icon
local volume = wibox.widget{
    font = beautiful.icon_var .. "12",
    markup = helpers.colorize_text("  ", beautiful.fg_color),
    widget = wibox.widget.textbox,
    valign = "center",
    align = "center"
}

local function updateVolumeIcon(volume_level)
    -- Modify this logic based on your specific volume icon set
    if volume_level >= 45 then
        volume.markup = helpers.colorize_text("󰕾 ", beautiful.fg_color)
    elseif volume_level >= 40 then
        volume.markup = helpers.colorize_text("󰕾 ", beautiful.fg_color)
    elseif volume_level >= 20 then
        volume.markup = helpers.colorize_text("󰖀 ", beautiful.fg_color)
    elseif volume_level >= 1 then
        volume.markup = helpers.colorize_text("󰕿 ", beautiful.fg_color)
    else
        volume.markup = helpers.colorize_text("󰸈 ", beautiful.fg_color .. "99")
    end
end

-- Example function to retrieve volume level
local function getVolumeLevel()
    local command = "amixer -D pulse sget Master"
    local handle = io.popen(command)
    local result = handle:read("*a")
    handle:close()

    -- Extract the volume percentage from the result
    local volumeLevel = tonumber(string.match(result, "(%d?%d?%d)%%"))

    -- Return the volume level (a number between 0 and 100)
    return volumeLevel or 0
end


-- Update volume icon initially
local initialVolumeLevel = getVolumeLevel()
updateVolumeIcon(initialVolumeLevel)

-- Update volume icon periodically
local volumeUpdateInterval = 0.5  -- Interval in seconds
awful.widget.watch("bash -c 'sleep " .. volumeUpdateInterval .. "; exit 0'", volumeUpdateInterval, function()
    local volumeLevel = getVolumeLevel()
    updateVolumeIcon(volumeLevel)
end)

-- battery indicator
local battery = wibox.widget{
    widget = wibox.container.arcchart,
    max_value = 100,
    min_value = 0,
    value = 50,
    thickness = dpi(3),
    rounded_edge = true,
    bg = beautiful.green_color .. "4D",
    colors = { beautiful.green_color},
    start_angle = math.pi + math.pi / 2,
    forced_width = dpi(17),
    forced_height = dpi(17)
}

-- indicator for cc. (control center)
local indicator = wibox.widget{
    widget = wibox.container.background,
    bg = beautiful.fg_color .. "4D",
    forced_height = dpi(2),
    visible = false
}

-- make it more cool!
local kaka = require("helpers.widgets.create_button")(
    {
        {
            nil, nil, indicator,
            layout = wibox.layout.align.vertical
        },
        {
            {
                volume,
                blue,
                wifi,
                battery,
                layout = wibox.layout.fixed.horizontal,
                spacing = dpi(15)
            },
            margins = {left = dpi(12), right = dpi(12)},
            widget = wibox.container.margin
        },
        layout = wibox.layout.stack
    },
    beautiful.bg_3,
    beautiful.fg_color .. "33",
    dpi(0),
    dpi(0),
    dpi(0),
    helpers.rrect(beautiful.rounded - 2)
)

-- the final wrapped box
local widget_box = wibox.widget{
    widget = kaka
}


-- update ind status
--------------------
awesome.connect_signal("control_center::visible", function(val) 
    if val then 
        indicator.visible = true
    else
        indicator.visible = false
    end
end)

-- toggle cc (control center) on press
widget_box:connect_signal(
    "button::press",
    function()
        cc_toggle()
    end
)


-- update widgets
-----------------

-- bluetooth
local bluetooth_active = false
local bluetooth_connected = false

local function updateBluetoothIcon()
    if bluetooth_active then
        if bluetooth_connected then
            blue.markup = helpers.colorize_text("󰂱 ", beautiful.fg_color)
        else
            blue.markup = helpers.colorize_text("󰂯 ", beautiful.fg_color)
        end
    else
        blue.markup = helpers.colorize_text("󰂲 ", beautiful.fg_color .. "99")
    end
end

local function isBluetoothActive()
    local cmd = "bluetoothctl show | grep 'Powered:' | awk '{print $2}'"
    awful.spawn.easy_async_with_shell(cmd, function(stdout)
        local output = string.gsub(stdout, '^%s*(.-)%s*$', '%1')
        if output == "yes" then
            bluetooth_active = true
        else
            bluetooth_active = false
        end
        updateBluetoothIcon()
    end)
end

local function checkConnectedDevices()
    local cmd = "bluetoothctl info | grep 'Connected: yes' -c"
    awful.spawn.easy_async_with_shell(cmd, function(stdout)
        local output = string.gsub(stdout, '^%s*(.-)%s*$', '%1')
        if tonumber(output) == 1 then
            bluetooth_connected = true
        else
            bluetooth_connected = false
        end
        updateBluetoothIcon()
    end)
end

-- Initial check
isBluetoothActive()
checkConnectedDevices()

-- Update Bluetooth status and connected devices every 5 seconds
awful.widget.watch("bash -c 'sleep 5; exit 0'", 5, function()
    isBluetoothActive()
    checkConnectedDevices()
end)

-- wifi
awesome.connect_signal("signal::wifi", function (value)
    if value then
        wifi.markup = helpers.colorize_text("󰖩 ", beautiful.fg_color)
    else 
        wifi.markup = helpers.colorize_text("󰖪 ", beautiful.fg_color .. "99")
    end
end)

-- battery
awesome.connect_signal("signal::battery", function(value) 
    battery.value = value
end)


-- finalize
-----------
return widget_box

-- eof
------